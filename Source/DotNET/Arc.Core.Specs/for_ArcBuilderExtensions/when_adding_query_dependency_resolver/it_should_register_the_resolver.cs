...
{
    void Because() => _builder.WithQueryDependencyResolver<TestQueryDependencyResolver>();

    [Fact] void should_register_resolver_service_type() =>
        _services.ShouldContain(_ => _.ServiceType == typeof(IQueryDependencyResolver) && _.ImplementationType == typeof(TestQueryDependencyResolver));

    [Fact] void should_register_as_singleton() =>
        _services.ShouldContain(_ => _.ServiceType == typeof(IQueryDependencyResolver) && _.Lifetime == ServiceLifetime.Singleton);
}

file class TestQueryDependencyResolver : IQueryDependencyResolver
{
