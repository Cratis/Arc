...
public class and_provide_takes_nullable_dependency_that_resolves_to_null : given.a_command_pipeline
{
    record ProvidedDependencyState(bool WasNull);
    ...
    class Command
    {
        ...
        public ProvidedDependencyState Provide(Dependency? dependency) => new(dependency is null);

        public void Handle(ProvidedDependencyState state) => DependencyWasNull = state.WasNull;
    }
    ...
    [Fact] void should_inject_null_into_provide() => _command.DependencyWasNull.ShouldBeTrue();
}
