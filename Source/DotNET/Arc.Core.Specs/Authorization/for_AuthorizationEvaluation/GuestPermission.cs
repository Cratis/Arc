...
public class GuestPermission : IAuthorizationPolicy
{
    public ValueTask<bool> IsAuthorized(AuthorizationPolicyContext context, CancellationToken cancellationToken) =>
        ValueTask.FromResult(context.Principal.Identity?.IsAuthenticated != true && context.Target != typeof(RejectedGuestCommand));
}
