...
public class when_subscribing_to_an_interface_without_declared_identity : given.an_observable_query_demultiplexer
{
    ...
    [Fact] void should_not_warn_when_concrete_items_have_identity() => _observedLogger.Entries.Count(_ => _.Level == LogLevel.Warning).ShouldEqual(0);
    [Fact] void should_send_the_initial_empty_snapshot() => ((IEnumerable<IReadModel>)_results[0].Data).Count().ShouldEqual(0);
    [Fact] void should_send_a_delta_for_the_first_item() => _results[1].ChangeSet!.Added.Count().ShouldEqual(1);
    [Fact] void should_send_a_delta_for_the_second_item() => _results[2].ChangeSet!.Added.Count().ShouldEqual(1);
    [Fact] void should_omit_full_data_on_later_emissions() => _results[2].Data.ShouldBeNull();
}
