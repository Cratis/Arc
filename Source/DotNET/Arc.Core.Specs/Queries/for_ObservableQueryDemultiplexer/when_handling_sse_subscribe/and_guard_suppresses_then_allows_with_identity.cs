...
/// <summary>
/// A suppressed emission must not move the delta baseline: the next delivered change set includes everything
/// that changed since the last state the client actually received.
/// </summary>
public class and_guard_suppresses_then_allows_with_identity : given.a_guarded_sse_connection
{
    record Item(string Id);

    readonly Subject<IEnumerable<Item>> _items = new();

    void Establish()
    {
        _streamingData = _items;
    ...
    {
        _items.OnNext([new Item("item-a")]);
        await WaitFor(() => _guardCalls.Count == 1);

        _items.OnNext([new Item("item-a"), new Item("item-b")]);
        await WaitFor(() => _guardCalls.Count == 2);

        _items.OnNext([new Item("item-a"), new Item("item-b"), new Item("item-c")]);
