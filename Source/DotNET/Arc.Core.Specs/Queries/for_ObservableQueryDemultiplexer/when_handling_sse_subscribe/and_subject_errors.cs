...
public class and_subject_errors : given.a_guarded_sse_connection
{
    readonly CompletionState _state = new();
    bool _hadObserversAfterError;
    int _errorsAfterTermination;
    ...
    async Task Because() => await RunConnection(async () =>
    {
        ...
        _subject.OnError(new TestFailure("producer failed"));
        await WaitFor(() => HasErrorFor(FirstQueryId));
        await _state.ScopeDisposed.Task.WaitAsync(TimeSpan.FromSeconds(2));
        _hadObserversAfterError = _subject.HasObservers;
        _errorsAfterTermination = HubMessages.Count(_ =>
    ...
    [Fact] void should_detach_the_subject_observer() => _hadObserversAfterError.ShouldBeFalse();
    ...
    [Fact] void should_not_send_a_late_error() =>
        HubMessages.Count(_ => _.Type == ObservableQueryHubMessageType.Error && _.QueryId == FirstQueryId).ShouldEqual(1);
