...
public class when_setting_severity_to_information : Specification
{
    ...
    class TestCommandValidator : BaseValidator<TestCommand>
    {
        ...
        {
            RuleFor(x => x.Name).NotEmpty().WithSeverity(ValidationResultSeverity.Information);
        }
    }
    ...
    [Fact] void should_have_info_severity() => _result.Errors[0].Severity.ShouldEqual(Severity.Info);
}
