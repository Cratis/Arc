...
public class when_executing_controller_command_with_nullable_properties : given.a_scenario_web_application
{
    CommandResult<string>? _result;

    void Establish() => LoadControllerCommandProxy<ControllerCommandsController>(nameof(ControllerCommandsController.ExecuteWithNullableProperties));

    async Task Because()
    {
        var executionResult = await Bridge.ExecuteCommandViaProxyAsync<string>(new ControllerCommandWithNullableProperties
        {
            Name = "TestName"
        },
            "ExecuteWithNullableProperties");
