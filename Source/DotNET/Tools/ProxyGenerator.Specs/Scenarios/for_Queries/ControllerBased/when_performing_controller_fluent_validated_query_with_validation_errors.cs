...
public class when_performing_controller_fluent_validated_query_with_validation_errors : given.a_scenario_web_application
{
    ...
    void Establish() => ControllerQueriesController.FluentValidatedCallCount = 0;

    async Task Because()
    {
        var response = await HttpClient.GetAsync("/api/controller-queries/fluent-validated?email=invalid-email&minAge=200");
