...
/// chosen to be right without configuration - a <see cref="DateOnly"/> is a calendar date, so it crosses as the
/// <c>DateOnly</c> in <c>@cratis/fundamentals</c> rather than as a JavaScript <c>Date</c> that would invent an
/// instant for it. The mapping seam then lets a consumer who wants a different type of their own say so.
/// </summary>
/// <remarks>
...
/// wants a type of their own. A default that could not be overridden would force a fork; a seam that had to be
/// configured before the common case was correct would be a wrong default with extra steps.
/// </remarks>
...
public class when_a_type_mapping_replaces_the_temporal_default : given.no_type_mappings
{
    ...
    [Fact] void should_default_a_calendar_date_to_the_calendar_date_type() => _dateOnlyByDefault.Type.ShouldEqual("DateOnly");
    [Fact] void should_default_a_time_of_day_to_the_time_of_day_type() => _timeOnlyByDefault.Type.ShouldEqual("TimeOnly");
    [Fact] void should_import_the_default_from_fundamentals() => _dateOnlyByDefault.Module.ShouldEqual("@cratis/fundamentals");
    ...
    /// move the other.
    /// </summary>
    [Fact] void should_leave_an_unmapped_sibling_alone() => typeof(TimeOnly).GetTargetType().Type.ShouldEqual("TimeOnly");
}
