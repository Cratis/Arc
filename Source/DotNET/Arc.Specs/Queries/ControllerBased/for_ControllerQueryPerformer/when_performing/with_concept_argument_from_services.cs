...
public class with_concept_argument_from_services : Specification
{
    ...
    [Fact] void should_resolve_the_concept_from_services() => ((RateLookup)_result!).Value.ShouldEqual(42m);
    [Fact] void should_not_expose_the_concept_as_a_query_parameter() => _performer.Parameters.Any(p => p.Name == "rate").ShouldBeFalse();
    ...
    {
        [HttpGet("by-rate")]
        public RateLookup ByRate([FromServices] Rate rate) => new(rate.Value);
    }
}
