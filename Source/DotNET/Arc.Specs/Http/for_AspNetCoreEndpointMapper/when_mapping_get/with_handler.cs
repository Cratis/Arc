...
{
    const string Pattern = "/test/get";
    RouteEndpoint _endpoint;

    void Establish()
    {
        _mapper.MapGet(Pattern, _ => Task.CompletedTask);
    }
    ...
    [Fact] void should_have_get_http_method() => _endpoint.Metadata.GetMetadata<HttpMethodMetadata>().HttpMethods.ShouldContain("GET");
}
