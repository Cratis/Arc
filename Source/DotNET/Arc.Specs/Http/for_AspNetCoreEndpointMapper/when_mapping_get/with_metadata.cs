...
{
    const string Pattern = "/test/get-meta";
    const string EndpointName = "TestGetEndpoint";
    const string Summary = "A test GET endpoint";
    RouteEndpoint _endpoint;

    void Establish()
    {
        var metadata = new EndpointMetadata(EndpointName, Summary, ["TestTag"], false);
        _mapper.MapGet(Pattern, _ => Task.CompletedTask, metadata);
    }
