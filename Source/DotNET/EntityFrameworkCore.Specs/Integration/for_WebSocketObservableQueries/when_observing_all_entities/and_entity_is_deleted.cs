...
public class and_entity_is_deleted : a_running_arc_application_with_observable_queries
{
    ...
    async Task Because()
    {
        ...
        // Wait for notification where the deleted entity is no longer present
        _receivedDeleteNotification = await _webSocketClient.WaitForMessage(
            result =>
            {
                var data = result.Data as JsonElement?;
                if (data?.ValueKind == JsonValueKind.Array)
                {
                    var hasDeletedEntity = false;
                    foreach (var item in data.Value.EnumerateArray())
                    {
                        if (item.TryGetProperty("name", out var nameProp) ||
                            item.TryGetProperty("Name", out nameProp))
                        {
                            if (nameProp.GetString() == EntityToDelete)
                            {
                    ...
                    // Return true when the deleted entity is no longer in the array
                    // and we have at least received a response (after the initial one)
                    return !hasDeletedEntity && _webSocketClient.MessageCount > _initialMessageCount;
                }
                return false;
            },
    ...
    [Fact] void should_receive_notification_without_deleted_entity() => _receivedDeleteNotification.ShouldBeTrue();
