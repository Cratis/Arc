...
public class when_adding_readonly_db_context_with_connection_string : Specification
{
    IServiceProvider _serviceProvider;
    TestReadOnlyDbContext _dbContext;

    void Establish()
    {
        var services = new ServiceCollection();
        services.AddSingleton(Substitute.For<ITypes>());
        services.AddReadOnlyDbContextWithConnectionString<TestReadOnlyDbContext>("Data Source=:memory:");

        _serviceProvider = services.BuildServiceProvider();
        _dbContext = _serviceProvider.GetRequiredService<IDbContextFactory<TestReadOnlyDbContext>>().CreateDbContext();
    }
