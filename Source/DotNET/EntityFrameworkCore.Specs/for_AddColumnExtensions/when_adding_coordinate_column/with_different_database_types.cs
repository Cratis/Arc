...
namespace Cratis.Arc.EntityFrameworkCore.for_AddColumnExtensions.when_adding_coordinate_column;
...
public class with_different_database_types : given.a_migration_builder
{
    ...
    void Because()
    {
        _sqliteMb.AddCoordinateColumn("Location", "TestTable");
        _sqliteResult = (AddColumnOperation)_sqliteMb.Operations[0];
        _sqlServerMb.AddCoordinateColumn("Location", "TestTable");
        _sqlServerResult = (AddColumnOperation)_sqlServerMb.Operations[0];
        _postgreSqlMb.AddCoordinateColumn("Location", "TestTable");
