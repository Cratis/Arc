...
public class with_decimal_type : given.a_migration_builder
{
    ...
    void Because()
    {
        _sqliteMb.AddNumberColumn<decimal>("Price", "TestTable");
        _sqliteResult = (AddColumnOperation)_sqliteMb.Operations[0];
        _sqlServerMb.AddNumberColumn<decimal>("Price", "TestTable");
        _sqlServerResult = (AddColumnOperation)_sqlServerMb.Operations[0];
        _postgreSqlMb.AddNumberColumn<decimal>("Price", "TestTable");
        _postgreSqlResult = (AddColumnOperation)_postgreSqlMb.Operations[0];
    }

    [Fact] void should_set_sqlite_type_to_real() => _sqliteResult.ColumnType.ShouldEqual("REAL");
    [Fact] void should_set_sql_server_type_to_decimal() => _sqlServerResult.ColumnType.ShouldEqual("DECIMAL(18,2)");
    [Fact] void should_set_postgresql_type_to_decimal() => _postgreSqlResult.ColumnType.ShouldEqual("DECIMAL");
}
