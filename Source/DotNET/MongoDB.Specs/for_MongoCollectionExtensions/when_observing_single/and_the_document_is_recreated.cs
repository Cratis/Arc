...
/// <summary>
/// The document is deleted and then a new document with the same id is inserted. Proves the observer is not
/// left dead after reporting "gone" — it keeps tracking the same id and emits again once it reappears.
/// </summary>
public class and_the_document_is_recreated : given.an_observed_collection
{
    ...
    ObservedDocument? _afterRecreateEmission;
    ...
    async Task Because()
    {
        ...
        PushChange(InsertOf(_target));
        _afterRecreateEmission = await _emissions.Next(TimeSpan.FromSeconds(5));
    }
    ...
    [Fact] void should_emit_the_document_again_once_recreated() => _afterRecreateEmission.ShouldEqual(_target);
}
