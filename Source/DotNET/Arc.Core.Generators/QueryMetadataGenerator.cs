...
public class QueryMetadataGenerator : IIncrementalGenerator
{
    ...
    static void GenerateSource(SourceProductionContext context, ImmutableArray<ReadModelInfo> readModels)
    {
        ...
        foreach (var typeName in readModels.Select(r => r.FullyQualifiedTypeName).Distinct())
        {
            sb.Append("    [DynamicDependency(DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.PublicProperties, typeof(global::")
              .Append(typeName)
              .AppendLine("))]");
        }
