...
public class and_reactor_appends_through_an_event_store_property : Specification
{
    Exception _result;
...
namespace TestNamespace
{
    ...
    {
        public AuthorNotifier(IEventStore eventStore) => Store = eventStore;

        IEventStore Store { get; }

        public Task On(AuthorRegistered @event) =>
            {|#0:Store.EventLog|}.Append(""some-author"", @event);
    }
}",
                VerifyCS.Diagnostic("ARCCHR0003")
                    .WithSeverity(DiagnosticSeverity.Warning)
                    .WithLocation(0)
                    .WithArguments("AuthorNotifier", "Store.EventLog")));
