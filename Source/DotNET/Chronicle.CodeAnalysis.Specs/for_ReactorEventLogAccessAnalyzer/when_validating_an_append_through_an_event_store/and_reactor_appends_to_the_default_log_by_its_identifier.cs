...
public class and_reactor_appends_to_the_default_log_by_its_identifier : Specification
{
    Exception _result;
...
namespace TestNamespace
{
    ...
    public class AuthorNotifier(IEventStore eventStore) : IReactor
    {
        public Task On(AuthorRegistered @event) =>
            {|#0:eventStore.GetEventSequence(""event-log"")|}.Append(""some-author"", @event);
    }
}",
                VerifyCS.Diagnostic("ARCCHR0003")
                    .WithSeverity(DiagnosticSeverity.Warning)
                    .WithLocation(0)
                    .WithArguments("AuthorNotifier", "eventStore.GetEventSequence(\"event-log\")")));
