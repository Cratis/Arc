...
public class and_reactor_appends_to_another_event_store_inline : Specification
{
    Exception _result;
...
namespace TestNamespace
{
    ...
    public class AuthorNotifier(IChronicleClient client) : IReactor
    {
        public async Task On(AuthorRegistered @event) =>
            await (await client.GetEventStore(""Other"", ""tenant-x"")).EventLog.Append(""some-author"", @event);
    }
}"));
