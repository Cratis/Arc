    ...
    where TAnalyzer : DiagnosticAnalyzer, new()
{
    ...
    static void VerifyDiagnostic(Diagnostic diagnostic, ExpectedDiagnostic expected, IReadOnlyDictionary<int, TextSpan> markers, int index)
    {
        ...
        {
            if (!diagnosticMessage.Contains(argument, StringComparison.Ordinal))
            {
                throw new Exception($"Diagnostic message does not contain expected argument.\nExpected argument: {argument}\nActual message: {diagnosticMessage}");
            }
        }
