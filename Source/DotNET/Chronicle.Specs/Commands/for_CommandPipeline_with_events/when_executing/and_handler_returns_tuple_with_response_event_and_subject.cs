...
public class and_handler_returns_tuple_with_response_event_and_subject : given.a_command_pipeline_with_event_handlers_and_command
{
    ...
    void Establish()
    {
        ...
        _subject = new Subject("customer-84");
        _commandHandler.Handle(Arg.Any<CommandContext>()).Returns((_response, _event, _subject));
    }
    ...
    [Fact] void should_append_event_to_event_log() => _eventLog.Received(1).Append(
        ...
        _subject);
    [Fact] void should_return_response_value() => _result.Response.ShouldEqual(_response);
