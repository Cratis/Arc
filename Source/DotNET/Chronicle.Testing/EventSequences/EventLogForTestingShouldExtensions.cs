...
/// <summary>
/// Provides assertion extension methods for <see cref="EventLogForTesting"/>.
/// </summary>
public static class EventLogForTestingShouldExtensions
{
    /// <summary>
    /// Asserts that the tail sequence number of the event log matches the expected value.
    /// </summary>
    /// <param name="eventLog">The <see cref="EventLogForTesting"/> to assert on.</param>
    ...
    /// <exception cref="EventLogAssertionException">Thrown when the tail sequence number does not match.</exception>
    public static async Task ShouldHaveTailSequenceNumber(this EventLogForTesting eventLog, EventSequenceNumber expected)
    {
        var actual = await eventLog.GetTailSequenceNumber(
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous assertion.</returns>
    /// <exception cref="EventLogAssertionException">Thrown when the event is not found or has the wrong type.</exception>
    public static Task ShouldHaveAppendedEvent<TEvent>(
        this EventLogForTesting eventLog,
        EventSequenceNumber sequenceNumber) =>
        eventLog.ShouldHaveAppendedEvent<TEvent>(sequenceNumber, _ => { });
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous assertion.</returns>
    /// <exception cref="EventLogAssertionException">Thrown when the event is not found, has the wrong type, or the validator throws.</exception>
    public static async Task ShouldHaveAppendedEvent<TEvent>(
        this EventLogForTesting eventLog,
        EventSequenceNumber sequenceNumber,
        Action<TEvent> validator)
    {
        var events = await eventLog.GetFromSequenceNumber(sequenceNumber, EventSourceId.Unspecified, []);
        AssertEventInList(events, sequenceNumber, validator);
    }

    /// <summary>
    /// Asserts that a specific event type was appended for the given event source at the given sequence number.
    /// </summary>
    /// <typeparam name="TEvent">The expected event type.</typeparam>
    /// <param name="eventLog">The <see cref="EventLogForTesting"/> to assert on.</param>
    /// <param name="sequenceNumber">The <see cref="EventSequenceNumber"/> at which the event is expected.</param>
    ...
    public static async Task ShouldHaveAppendedEvent<TEvent>(
        ...
        Action<TEvent> validator)
    {
        var events = await eventLog.GetFromSequenceNumber(sequenceNumber, eventSourceId, []);
        AssertEventInList(events, sequenceNumber, validator);
    }
