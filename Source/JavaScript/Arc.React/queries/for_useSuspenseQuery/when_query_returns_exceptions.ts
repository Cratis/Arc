...
describe('when query returns exceptions', () => {
    ...
    beforeEach(() => {
        ...
        fetchStub.resolves({
            json: async () => ({
                ...
                hasExceptions: true,
                validationResults: [],
                exceptionMessages: ['Internal server error', 'Stack overflow'],
    ...
    it('should propagate a QueryFailed error to the error boundary', async () => {
        ...
        (capturedError as unknown as QueryFailed).exceptionMessages.should.deep.equal(['Internal server error', 'Stack overflow']);
    });
});
