...
describe(
    'when a retired EventSource fires connected and error callbacks',
    given(a_server_sent_event_hub_connection, (context) => {
        ...
        beforeEach(() => {
            ...
            const reconnect = (context.policy.schedule as sinon.SinonStub).firstCall
                .args[0] as () => void;
            reconnect();
        ...
        it('should not send another subscription for stale connected', () =>
            context.fetchStub.callCount.should.equal(2));
        it('should keep the current EventSource open', () =>
            currentSource.close.called.should.be.false);
        it('should not schedule another reconnect for stale error', () =>
