...
import { a_deferred_promise } from '../given/a_deferred_promise.js';
...
describe(
    'when a query is unsubscribed while its subscribe request is in flight and the connection changes',
    given(a_server_sent_event_hub_connection, (context) => {
        ...
        it('should not subscribe the removed query on connection B', () => {
            const removedQueryCalls = context.fetchStub
                .getCalls()
                .filter((call) => call.args[0] === subscribeUrl)
                .filter((call) =>
                    (call.args[1].body as string).includes('"queryId":"removed-query"'),
                );
        ...
        it('should subscribe the remaining query on connection B', () => {
            const activeQueryBodies = context.fetchStub
                .getCalls()
                .filter((call) => call.args[0] === subscribeUrl)
                .map((call) => parseBody(call.args[1].body as string))
                .filter((body) => body.queryId === 'active-query');
            activeQueryBodies
                .map((body) => body.connectionId)
                .should.deep.equal(['connection-a', 'connection-b']);
            activeQueryBodies[0].revision.should.equal(activeQueryBodies[1].revision);
        });

        it('should not reconnect', () =>
            (context.policy.schedule as sinon.SinonStub).called.should.be.false);

        it('should retain only the remaining subscription', () =>
