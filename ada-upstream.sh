#!/usr/bin/env bash
# Launch a Claude Code session on Ada's upstream backlog for Arc.
#
...
cd "$REPO_ROOT"
echo "==> Arc  ($(git rev-parse --abbrev-ref HEAD))  item $ITEM"
exec claude --add-dir "$ADA/Planning" "Read $REPO_ROOT/ADA-UPSTREAM.md in full before doing anything else. It is Ada's upstream backlog for this repo: 13 open items - 7 defect reports (fixes) and 6 improvement proposals (features and missing seams). Work item $ITEM end to end, following the discipline section of that file: re-verify every cited file:line at HEAD, reproduce at the right tier before fixing, mutation-prove the spec, run this repo's full gates with zero warnings, then report which report claims you confirmed, refuted or corrected. Do not start another item until I say so. /Volumes/sourcecode/repos/hive/Ada is READ-ONLY reference material - never create, edit or delete anything under it. Work on a branch. Do not push and do not open a PR."
