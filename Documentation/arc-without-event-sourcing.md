...
These direct collection calls remain supported. On the C# backend, for immediate inline work that a model-bound command can describe before execution, prefer returned [command operations](./backend/csharp/commands/operations/index.md). The JVM backend has no operation mechanism; there, perform the work in `handle` through an injected service. `Handle()` declares the write and Arc performs it afterward, without requiring Chronicle. Optional compensation is best-effort and in-process, not atomic database rollback or durable recovery. The [migration recipe](./backend/csharp/commands/operations/migrating.md) shows how to preserve an existing response contract.
:::
