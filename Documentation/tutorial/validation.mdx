...
description: Reject a blank or duplicate author name — first with a validator on the value type, then with a business rule that checks existing state — and watch the reason appear in the React form for free.
---
...
A check-then-write has a race: two registrations at the same instant can both pass the validator. The validator is for a *friendly message*; back it with a real **unique index** on the name (a MongoDB unique index, or an EF `HasIndex(...).IsUnique()`) so the database is the final authority. When history and concurrency matter at this level, it's also exactly the kind of guarantee [event sourcing with Chronicle](./add-event-sourcing) makes airtight — that's chapter 6.
</Aside>
...
- An `AuthorNameValidator` that guards the **value type**, enforced everywhere an `AuthorName` is used and run client-side for free.
