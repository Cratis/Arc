...
| [Use current state in a command](./use-current-state-in-a-command.md) | A decision depends on projected state — inject the read model instead of querying for it |
...
Event-sourced Arc slices have their own Chronicle-specific recipes, starting with [React to an event](/arc/backend/chronicle/react-to-an-event/) and [Add event sourcing to an Arc slice](/arc/backend/chronicle/add-event-sourcing/).
