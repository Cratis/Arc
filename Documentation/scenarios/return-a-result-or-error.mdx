...
The following **handler replacement** uses the [MongoDB tutorial's setup and domain types](/arc/backend/csharp/getting-started/your-first-command/). On C#, add `using Cratis.Monads;` and `using Cratis.Arc.Validation;` to the command file:

<ArcBackendTabs snippet="scenarios/return-a-result-or-error/handle-result" />

The two backends reject differently, and it is worth being explicit about it. C# declares the alternative in the signature with `Result<TResponse, ValidationResult>`. Arc.Kotlin has no `Result<,>` type and no handler return shape that carries both a client response type and a rejection, because the generated proxy and OpenAPI metadata need exactly one client response leaf. The JVM way is to throw an application exception implementing `io.cratis.arc.validation.ValidationFailure`: the command pipeline converts it into the same validation feedback a validator would produce, without leaking the exception message or stack trace. An ordinary exception still becomes an exception result.
...
For rules that can reject before work begins, prefer a `CommandValidator<T>`; use a typed handler failure when the write/decision itself discovers the failure. `Provide()` can also [short-circuit with typed validation](./provide-data-to-a-command.mdx).

:::note[Separate an already-decided write from its response]
When the work and caller response are already decided, the C# backend can return a [command operation](../backend/csharp/commands/operations/index.md) alongside the ordinary response. Direct service calls remain supported when their result is needed to choose the response or typed rejection. Operation `Execute()` methods cannot return receipts or validation alternatives; moving a write into an operation does not automatically preserve write-dependent error handling.
:::
...
- [Validate a command](./validate-a-command.mdx) — choose the right rejection point.
- [Test a command](./test-a-command.mdx) — assert the actual write, not just a successful result.
