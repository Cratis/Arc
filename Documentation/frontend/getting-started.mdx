...
And the form is wired to the live list for nothing extra — `AllAuthors.use()` is subscribed to the read model, so the moment the command appends its event and the projection updates, the list re-renders. You didn't write any of that plumbing.
