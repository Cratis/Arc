...
| `autoServerValidate` | `false` | Enables automatic server preflight; see [Auto server validation](./auto-server-validation.mdx) for request paths and timing. |
...
This standalone Arc endpoint is a complete command-and-validator example for an already configured Arc host, in whichever backend language you use. It returns the accepted values; it deliberately does **not** persist a profile. A real update also needs an explicit backend write. No event store or Chronicle integration is involved.
...
<ArcBackendTabs snippet="frontend/react/command-form/validation/profile-command" />

The rules land in different places because the two backends project different surfaces to the client. C# extracts a supported subset from FluentValidation — here a `ConceptValidator<EmailAddress>` that travels with the value and a `CommandValidator<UpdateProfile>` for the command's own rule. Arc.Kotlin's client-projected surface is [shared fluent validation](/arc/backend/kotlin/guides/validation/): a `FluentModelValidator<T>` whose single `init` block declares literal rules over string, collection, and numeric members, which is why the JVM command carries plain `String` members rather than concepts. Anything conditional, asynchronous, or service-dependent stays in an ordinary `CommandValidator` there, server-side.
...
- `isValid` requires the latest applied **silent** result to have an empty `validationResults` array and no nonempty custom field errors to remain. It does not come from displayed `commandResult.isValid`. Even a warning in silent validation makes that form flag false; command execution has its own severity policy.
...
- Nonempty errors from `onFieldValidate` and `setCustomFieldError` block form execution and are reported as validation failures. Clearing the last custom error restores retained native feedback; see [custom-error result behavior](./form-lifecycle.md#command-result-callbacks). Raw command execution bypasses this form-local gate, so enforce business rules in command/server validation too.
...
- [Auto server validation](./auto-server-validation.mdx)
