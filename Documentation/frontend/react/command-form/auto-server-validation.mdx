...
This complete component uses the generated `UpdateProfile` from the [validation example](./validation.mdx#backend-validation). Configure Arc and generate the proxy before using it.
...
First define an actual endpoint, such as `UpdateProfile` in [Backend validation](./validation.mdx#backend-validation). A standalone DTO and validator alone are not this model-bound endpoint.

The following **application integration fragment** adds the directory rule beside that example's command; the generated client and the form behaviour below are the same on either backend. It compiles alongside `UpdateProfile`; to run it, supply and register your application's profile-directory implementation. The allowed-email check must apply your own profile/tenant policy, including allowing an unchanged email for an existing profile.

<ArcBackendTabs snippet="frontend/react/command-form/auto-server-validation/server-only-rule" />

In C#, keep the `EmailAddressValidator` from the linked example: it owns the required/format rules, and this replacement validator takes over the command-level ones. Arc's concept-aware `RuleFor` unwraps the property, so the three-argument `MustAsync` overload reads the named `command.Email` from the command when calling the application service. `MinimumLength` and `MaximumLength` are FluentValidation method names.

The shapes genuinely diverge here. Arc.Kotlin's shared fluent surface is deliberately literal and has **no** asynchronous or predicate rule, so the JVM sibling is an ordinary `CommandValidator` whose `validate` is already a suspending function, added beside the `FluentModelValidator` that keeps projecting the literal rules to the client. There is also no `CancellationToken` parameter on the JVM: a suspending validator runs inside Arc's command coroutine, so cancelling the request cancels what it awaits.

Either way the asynchronous directory rule remains server-side, while the name rules and supported format rules are generated for the client. No `minLength`, `maxLength`, or `pattern` props are supported on the built-in `InputTextField`.
...
- A previous validity verdict can remain while a new request runs. Internal issue tokens reject results overtaken by later applied validation, but do not guarantee every visible verdict describes the latest edit. For a custom gated UI, use the [revision-guarded pattern](./validation.mdx#progressive-validation).
