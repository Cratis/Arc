---
title: Commands
description: Execute backend commands from React through the generated proxy's .use() hook, with change tracking, validation, and typed results.
---

Commands represent actions you want to perform against the system. Arc generates their HTTP surface from model-bound commands, and in C# also from discovered controller actions. Either style can use validation and authorization; neither requires Chronicle or event sourcing.
...
**Backend fragment:** these are the declarations the proxy below is generated from.

<ArcBackendTabs snippet="frontend/react/commands/index/command-payload" />

The two backends reach the same generated command from different starting points. The C# record is a **payload DTO**: it requires an already-defined controller endpoint accepting it, because a record alone is not enough for proxy discovery. The JVM has no controller-based discovery, so the model-bound `@Command` *is* the endpoint. For a complete model-bound C# example, follow [getting started](/arc/frontend/react/getting-started/).
