...
and validation state alongside. When a command writes and the `AllAccounts` read
model changes, every browser observing this query re-renders with the new list — no refresh, no refetch. That
