...
| Named policies and authentication schemes | Partial | Implemented | Implemented | C# enforces authentication and roles on model-bound commands and read models, but not a named `Policy` or `AuthenticationSchemes`, and reports either with [`ARC0021`](https://github.com/Cratis/Arc/blob/main/Source/DotNET/Arc.Core.CodeAnalysis/DiagnosticDescriptors.cs) ([#2736](https://github.com/Cratis/Arc/issues/2736)). Java policies need an adapter bean. [C#](backend/csharp/core/authorization.md) · [Kotlin and Java](/arc/backend/kotlin/guides/security/) |
