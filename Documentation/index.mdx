...
Turn **commands** and **queries** into a full-stack CQRS application over MongoDB or EF Core, with **generated TypeScript proxies** that keep React in lockstep with your C#: no hand-written API client, no DTO duplication. [Build your first slice →](/arc/backend/getting-started/your-first-command/) · [Why Arc? →](/arc/why-arc/)
</TopicHero>
...
Arc feeds [Components](/components/) and runs over plain databases by default. When a slice needs event-sourced history, Arc can also sit on top of [Chronicle](/chronicle/) through its [optional Chronicle integration](/arc/backend/chronicle/). See [Why developers choose Cratis](/why-cratis/) for how the pieces fit.
