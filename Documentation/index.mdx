...
Use a .NET SDK compatible with your selected Arc release (the current source targets .NET 10). This is the **ASP.NET Core** path; [Arc.Core's lightweight host](/arc/backend/csharp/core/getting-started/) is a separate alternative.

<Steps>
1. Create an ASP.NET Core project and install the host package.
...
1. Run `dotnet run` and confirm the application starts. Continue with [standalone setup](/arc/backend/csharp/getting-started/) for database configuration, generated proxies, and the first command/query checkpoint.
</Steps>
