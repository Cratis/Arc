...
Turn **commands** and **queries** into a full-stack CQRS application — backed by [Chronicle](/chronicle/), MongoDB, or EF Core — with **generated TypeScript proxies** that keep React in lockstep with your C#: no hand-written API client, no DTO duplication. [Build your first slice →](/arc/backend/getting-started/your-first-command/) · [Why Arc? →](/arc/why-arc/)
</TopicHero>
...
<CardGrid>
  ...
  <SimpleCard title="Arc without event sourcing" icon="right-arrow" link="/arc/arc-without-event-sourcing/">
    Use Arc on its own over MongoDB or EF Core — the same typed full-stack app, no event store required.
  </SimpleCard>
...
Arc can sit on top of [Chronicle](/chronicle/) and feeds [Components](/components/) — though it [runs over a plain database just as well](/arc/arc-without-event-sourcing/). See [Why Cratis](/why-cratis/) for how the pieces fit.
