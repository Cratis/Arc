...
<CardGrid>
  ...
  <LinkCard
    ...
    href="/arc/coming-from-mediatr-and-mvc/"
  />
  <LinkCard
    title="Samples"
...
Use a .NET SDK compatible with your selected Arc release (the current source targets .NET 10). This is the **ASP.NET Core** path; [Arc.Core's lightweight host](/arc/backend/core/getting-started/) is a separate alternative.
...
1. Run `dotnet run` and confirm the application starts. Continue with [standalone setup](/arc/backend/getting-started/) for database configuration, generated proxies, and the first command/query checkpoint.
</Steps>
