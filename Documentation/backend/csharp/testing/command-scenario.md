...
The provider stays local to the scenario. It does not replace the process-wide provider an Arc host sets, so a scenario can run inside a live host or alongside other scenarios, and disposing it leaves the host untouched. Code that reads Arc's process-wide provider directly, such as the MongoDB and EF Core `Observe()` extensions, therefore sees the host's provider during a scenario, and throws `ServiceProviderNotConfigured` when no host has set one; command validation is unaffected because it resolves validators from the command's own scope.
