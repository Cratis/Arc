...
Register a named, scoped `IAuthorizationPolicy` as shown in [Core named policies](../../core/authorization.md#register-a-named-policy), then protect a command using `[Cratis.Arc.Authorization.Authorize(Policy = "ActiveSubscription")]`. Arc awaits the policy before running `Provide()` or `Handle()`. Stacked policy attributes all apply. An unknown or duplicate policy fails host startup rather than admitting callers. `AuthorizationPolicyContext.Resource` is the executing `CommandContext`; the policy sees the selected caller through `Principal`. Both contexts carry the same `ReceivedAt` (Arc receipt time). It does not change when policy evaluation is delayed or repeated. See [command context](../command-context.md#properties) for the capture boundary and validator access.

On an ASP.NET Core host, you can also use a named ASP.NET policy and `AuthenticationSchemes` (including schemes contributed by a policy). Arc authenticates the requested schemes, not just the default principal. A changed identity executes in a fresh Arc-owned service scope, so command values, execution-scope `Begin`, and handler dependencies bind to the selected tenant. An arbitrary explicit `IServiceProvider` may already hold dependencies for the default identity and is rejected for a scheme-selected command; use mapped HTTP or the command pipeline's scope-owning `Execute(command)` overload from a live request. Ordinary nested server-side commands with the same no-scheme actor remain supported; a nested scheme that changes identity after an outer command has started its unit of work is rejected before it can join that work. Every named scheme must be registered; a request without an authenticated selected scheme is denied. The standalone Core host rejects `AuthenticationSchemes` at startup (see [ARC0021](../../code-analysis/index.md#arc0021-unevaluated-authorization-settings)).

With `Cratis.Arc`, `Microsoft.AspNetCore.Authorization.AuthorizeAttribute` and `AllowAnonymousAttribute` on a model-bound command are enforced like Arc's own - authentication, roles, policies, and schemes. Arc's attributes are still the portable choice, because Arc Core's own HTTP host does not read the Microsoft ones; analyzer [ARC0020](../../code-analysis/index.md#arc0020-aspnet-core-authorization-attributes) reports a Microsoft attribute in a project without `Cratis.Arc`. Standard Microsoft authorization on **MVC controllers/actions**, or explicitly configured external middleware, is a separate enforcement path.
