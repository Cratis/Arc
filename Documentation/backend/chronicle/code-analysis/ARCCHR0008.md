...
These command fragments assume an application `Customer` read model and `CustomerRenamed` event; the example demonstrates key selection, not projection registration.
...
The mistake is invisible without it. The code compiles, and it reads exactly like a command that declares its key — but, unless another recognized identity source is present, Chronicle generates a fresh event source id instead of using that property. An existing-state lookup then usually finds nothing. A required dependency yields [`ReadModelDoesNotExistForCommand`](../read-models/failures.md#readmodeldoesnotexistforcommand) — "The command targets an entity that does not exist" — which points at the data rather than at the attribute, on a request whose entity exists perfectly well.
