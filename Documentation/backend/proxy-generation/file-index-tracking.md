...
3. **Index.ts Management**: `index.ts` is a **hand-owned file the generator merges into**, not a file it regenerates from scratch:
   - It only *adds* exports for files it generated in that same directory
   - It only *removes* an export when the relative path it points at no longer resolves on disk — a sibling `.ts`/`.tsx` file, or a sibling folder with its own `index.ts`/`index.tsx` barrel
   - Any export it cannot verify (a non-relative specifier, such as a re-exported package) is left untouched
   - Non-export lines (comments, custom imports) are preserved as-is
   - The `index.ts` file itself is deleted only if the directory becomes empty of generated content **and** has no surviving hand-written exports
...
`index.ts` is a file **you own**. The generator never rewrites it from scratch — it only ever *merges into* whatever is already there, and it does so conservatively:

- It **adds** an export only for a file it just generated in that same directory.
- It **removes** an export only when the relative path it points at no longer resolves to anything on disk.
- Anything it cannot verify as its own — a hand-written file, a hand-written sibling folder, a package re-export — is left exactly as you wrote it.
