...
description: Build a backend slice in Arc — a command that writes, a read model, and a live query that serves it — all in one folder, over a plain database.
---

import { Steps, Aside } from '@astrojs/starlight/components';

Let's build the backend half of a real feature: registering an author in a library app. It's small, but it exercises the whole Arc loop — something *happens* (a command), it writes state, and that state is served back out (a query) — over a plain database, with no event store required.
...
- **A database Arc can write to** — MongoDB, or EF Core over SQLite/Postgres. No event store is required for this slice.
...
- A `[ReadModel]` with a query method served over HTTP, live, for free.
...
This slice writes straight to MongoDB. Arc is persistence-agnostic — the same shape works over [EF Core](/arc/backend/entity-framework/getting-started/), and the day you want history, audit, and replay, you can move the write side to [Chronicle](/chronicle/) without touching your query or your frontend. The [tutorial's last chapter](/arc/tutorial/add-event-sourcing/) shows exactly that.
</Aside>
