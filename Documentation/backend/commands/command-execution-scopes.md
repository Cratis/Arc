...
| --- | --- | --- |
| `TransactionalCommandScope` | `Cratis.Arc.Chronicle` | Makes every command a [transactional scope](./transactional-commands.md): begins a Chronicle unit of work, commits it atomically when the command succeeds — surfacing constraint violations on the `CommandResult` — and rolls it back when the command fails. |
