...
namespace AspNetCore.Features.QueryShowcase;
...
public record ShowcaseItem(int Id, string Name, DateTimeOffset UpdatedAt)
{
    ...
    /// <summary>
    /// Query type 1: Observable, returns a single item — streams the latest showcase item via SSE.
    /// </summary>
    /// <returns>An observable emitting the latest item every 3 seconds.</returns>
    public static ISubject<ShowcaseItem> Latest() => _latest;

    /// <summary>
    /// Query type 2: Observable, returns a collection — streams the full list via SSE.
